...
loc <- loc[loc$Mount == 1,] # Only keep Magnet Mounts

# only keep locations in loc
observations <- observations[observations$OID %in% loc$OID,]
...
# Confirm location and effort matrix information
...
odds_ratio <- exp(summary(m42)$det$Estimate[2]) # Insert log odds ratio
odds_ratio
...
## Odds Ratios- Pipe Size and Impervious Surface
```{r BestModel_OR2, echo=TRUE}
test = summary(m36)
vcov_matrix <- vcov(m36)

# Occupancy- Distance to exit
# Calculate the odds ratio
odds_ratio <- exp(summary(m36)$state$Estimate[2]) # Insert log odds ratio
odds_ratio
...
# Occupancy- Pipe Size
# Calculate the odds ratio
odds_ratio <- exp(summary(m36)$state$Estimate[3]) # Insert log odds ratio
odds_ratio
...
nd_0_predict = predict(m42,type="state",newdata=nd_0,appendData=TRUE)
...
nd_1_predict = predict(m42,type="state",newdata=nd_1,appendData=TRUE)
...
nd_2_predict = predict(m42,type="state",newdata=nd_2,appendData=TRUE)
...
ggsave(P1, filename = "BestMod_Imp_Pipe.png", dpi = 300, width = 8, height = 6, units = "in")
